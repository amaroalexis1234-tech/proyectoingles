...
>(({ className, sideOffset = 8, align = "end", ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      align={align}
      className={cn(
        "animate-fade-in z-50 min-w-[12rem] rounded border border-border bg-surface p-1 shadow-card",
        className
      )}
      {...props}
    />
...
>(({ className, danger, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ref={ref}
    className={cn(
      "flex cursor-pointer items-center gap-2 rounded px-3 py-2 text-sm outline-none transition-colors",
      danger
        ? "text-error hover:bg-error/10 focus:bg-error/10"
        : "text-foreground hover:bg-background focus:bg-background",
      className
    )}
    {...props}
  />
